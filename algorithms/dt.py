...
def robust_top10_stats(rewards):
    rewards = np.array(rewards)
    sorted_rewards = np.sort(rewards)[::-1]  # Descending

    # Tukey's method for outlier filtering
    q1 = np.percentile(sorted_rewards, 25)
    q3 = np.percentile(sorted_rewards, 75)
    iqr = q3 - q1
    upper_bound = q3 + 1.5 * iqr

    filtered = sorted_rewards[sorted_rewards <= upper_bound]

    top_10 = filtered[:10] if len(filtered) >= 10 else filtered
...
def experiment(exp_prefix, variant):
    ...
    save_videos = variant.get('save_videos', False)  # Add option to save videos
    ...
    if env_name == 'hopper':
        ...
        env = Reacher2dEnv(render_mode='rgb_array')
        max_ep_len = 200
        env_targets = [76, 40]
        scale = 10.
    else:
    ...
    print('=' * 50)
    print(f'First Five Trajectory Lengths: {traj_lens[:5]}')
    print(f'First Five Returns: {returns[:5]}')
    ...
    def eval_episodes(target_rew):
        def fn(model):
            ...
            for episode in range(num_eval_episodes):
                with torch.no_grad():
                    if model_type == 'dt':
                        ...
                        if save_videos and ret > best_return:
                            best_return = ret
            ...
            if save_videos and best_frames is not None:
                video_path = os.path.join(
                    video_dir,
                    f"{env_name}_{dataset}_{target_rew}_best_ep{best_episode}_iter{trainer.current_iteration}.gif"
                )
                print(f"Saving best video to {video_path}")
                try:
            ...
            return {
                ...
                f'target_{target_rew}_return_top10_mean': robust_top10_stats(returns)['mean'],
                f'target_{target_rew}_return_top10_std': robust_top10_stats(returns)['std'],
            }
...
if __name__ == '__main__':
    ...
    parser.add_argument('--num_steps_per_iter', type=int, default=50000)  # Default = 10000
