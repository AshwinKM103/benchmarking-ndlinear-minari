...
def return_reward_range(dataset: Dict, max_episode_steps: int) -> Tuple[float, float, float]:
    returns, lengths = [], []
    ep_ret, ep_len = 0.0, 0
    dones = dataset["terminations"] | dataset["truncations"]
    for r, d in zip(dataset["rewards"], dones):
        ep_ret += float(r)
        ep_len += 1
    ...
    return min(returns), max(returns), avg_return
...
class MLP(nn.Module):
    def __init__(
        ...
        for i in range(n_dims - 2):
            layers.append(NdLinear(input_dims=(dims[i],), hidden_size=(dims[i + 1],)))
            layers.append(activation_fn())

            if dropout is not None:
                layers.append(nn.Dropout(dropout))
        layers.append(NdLinear(input_dims=(dims[-2],), hidden_size=(dims[-1],)))
    ...
    for t in range(int(config.max_timesteps)):
        ...
        if (t + 1) % config.eval_freq == 0:
            ...
            print(
                ...
                f"mean: {eval_score:.2f}, " f"std: {eval_scores.std():.2f}"
            )
            print("---------------------------------------")
            ...
            # wandb.log(
            #     {"Minari_normalized_score": normalized_eval_score},
